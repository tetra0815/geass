...
- After completing the constitution update, include a `Next Actions` section for each deferred
  intent. List the original intent and suggest the appropriate follow-up command, such
  as `/specify`, without invoking it.
...
- For each executable hook, output the following based on its `optional` flag:
  ...
  - **Mandatory hook** (`optional: false`):
    ...
    **Automatic Pre-Hook**: {extension}
    ...
    ```
    After emitting the block above you MUST actually invoke the hook and wait for it to finish before continuing. Run it the same way you would run the command yourself in this agent/session (the invocation may differ from the literal `{command}` id shown above, e.g. a skills-mode agent runs it as `/skill:constitution` or `$constitution`). Emitting the block alone does not run the hook.
- If no hooks are registered or `.geass/extensions.yml` does not exist, skip silently

## Outline

You are updating the project constitution at `.geass/memory/constitution.md`. This file is a TEMPLATE containing placeholder tokens in square brackets (e.g. `[PROJECT_NAME]`, `[PRINCIPLE_1_NAME]`). Your job is to (a) collect/derive concrete values and (b) fill the template precisely.

**Note**: If `.geass/memory/constitution.md` does not exist yet, it should have been initialized from `${CLAUDE_PLUGIN_ROOT}/templates/constitution-template.md` during project setup. If it's missing, copy the template first.

Follow this execution flow:

1. Load the existing constitution at `.geass/memory/constitution.md`.
...
6. Write the completed constitution back to `.geass/memory/constitution.md` (overwrite).
...
Do not create a new template; always operate on the existing `.geass/memory/constitution.md` file.
