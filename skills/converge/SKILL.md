...
- For each executable hook, output the following based on its `optional` flag:
  ...
  - **Mandatory hook** (`optional: false`):
    ...
    **Automatic Pre-Hook**: {extension}
    ...
    ```
    After emitting the block above you MUST actually invoke the hook and wait for it to finish before continuing. Run it the same way you would run the command yourself in this agent/session (the invocation may differ from the literal `{command}` id shown above, e.g. a skills-mode agent runs it as `/skill:{command}`). Emitting the block alone does not run the hook.
...
`/implement` can complete it. This command MUST run only after
`/implement` has run on the current `tasks.md`, and after `/tasks` has produced a complete `tasks.md`.
...
**Constitution Authority**: The project constitution (`.geass/memory/constitution.md`) is
...
- CONSTITUTION = `.geass/memory/constitution.md` (if present)
If `spec.md`, `plan.md`, or `tasks.md` is missing, STOP with a clear, actionable message naming the
prerequisite command to run (`/specify` for a missing spec, `/plan` for a missing plan,
`/tasks` for missing tasks). Do not produce partial output.
...
- On `tasks_appended`: state how many tasks were appended under which phase, and recommend
  running `/implement` to complete them; note that a follow-up converge
...
After producing the result, check if `.geass/extensions.yml` exists in the project root.
